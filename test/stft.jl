...
@test info.frequency_range == FreqRange(0, 4000)
...
@test_throws ArgumentError get_stft(audiofile; frequency_range=FreqRange(-100, 4000))
# test invalid frequency range - start >= end
@test_throws ArgumentError get_stft(audiofile; frequency_range=FreqRange(2000, 2000))
# test invalid frequency range - end > sr/2
@test_throws ArgumentError get_stft(audiofile; frequency_range=FreqRange(0, 5000))  # sr/2 = 4000
...
@test_throws ArgumentError get_stft(no_overlap_frames)
